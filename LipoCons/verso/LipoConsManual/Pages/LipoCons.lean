...
We prove the equivalence between {name sample_whole_space}`sample_whole_space` and {name is_consistent}`is_consistent` for any stochastic iterative global optimization {name Algorithm}`Algorithm` $`A`. This corresponds to *Proposition 3* of the paper _*{citehere Malherbe2017}[]*_.
