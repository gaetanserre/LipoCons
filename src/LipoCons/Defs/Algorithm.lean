...
import LipoCons.Utils.CompProd
...
by multiplying `μ` by the kernel that produces the next point conditioned on the current
history. The resulting measure is defined on `iter α n × α`, we then pull it back along the
measurable equivalence
`iter_mequiv α n : iter α (n + 1) ≃ᵐ iter α n × α` to obtain a measure on `iter α (n + 1)`.

The kernel is also pulled back along `prod_eval n f`, so that evaluations are
...
-- ANCHOR_END: eq_restrict
  ...
  | succ m hm =>
    ...
    intro B hB
    simp only [Measure.restrict_apply <| MeasurableSet.univ_pi hB]
    ...
    haveI : IsMarkovKernel (A.iter_comap (n := m) hf) := by
      simp only [iter_comap]
      infer_instance
    rw [Measure.compProd_apply' _ _ measurable_prod]

    haveI : IsMarkovKernel (A.iter_comap (n := m) hg) := by
      simp only [iter_comap]
      infer_instance
    rw [Measure.compProd_apply' _ _ measurable_prod]
