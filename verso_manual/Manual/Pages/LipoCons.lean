...
We prove the equivalence between {anchorTerm sample_iff_consistent}`sample_whole_space` and {anchorTerm sample_iff_consistent}`is_consistent_over_Lipschitz` for any stochastic iterative global optimization {anchorTerm sample_iff_consistent}`Algorithm` $`A`. This corresponds to *Proposition 3* of the paper _*{citehere Malherbe2017}[]*_.
