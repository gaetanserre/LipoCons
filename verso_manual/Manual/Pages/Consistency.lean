...
$$`
\max_{1 \le i \le n} f(X_i) \xrightarrow{p} \max_{x \in \mathcal{X}} f(x),
`
...
$$`
\forall \varepsilon > 0, \; \mathbb{P}(|\max_{1 \le i \le n} f(X_i) - \max_{x \in \mathcal{X}} f(x)| > \varepsilon) \xrightarrow[n \to \infty]{} 0,
`
where $`\mathbb{P}` denotes the probability measure on sequences of samples of size $`n + 1` produced by the algorithm. In our formalization, we showed that $`\mathbb{P}` is equal to `Algorithm.`{anchorTerm measure_dist_max}`measure`.

To formally define the consistency using `Algorithm.`{anchorTerm measure_dist_max}`measure`, we define the set of sequences of samples of size $`n + 1` such that the maximum of the evaluations is at least $`\varepsilon` away from the maximum of $`f` over the compact search space (noted {anchorTerm set_dist_max}`fmax`).
```anchor set_dist_max
...
$$`
\sup_{x \in \mathcal{X}} \min_{1 \le i \le n} \| x - X_i\| \xrightarrow{p} 0,
`
...
$$`
\forall \varepsilon > 0, \; \mathbb{P}(\sup_{x \in \mathcal{X}} \min_{1 \le i \le n} \| x - X_i\| > \varepsilon) \xrightarrow[n \to \infty]{} 0,
`
where $`\mathbb{P}` is equal to `Algorithm.`{anchorTerm measure_dist_max}`measure`.
